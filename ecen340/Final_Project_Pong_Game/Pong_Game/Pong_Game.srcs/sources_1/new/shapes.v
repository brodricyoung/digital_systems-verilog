...
module DRAW #(
    ...
    output [3:0] R, G, B,
    output [3:0] left_paddle_score, right_paddle_score
);

    reg [3:0] L_paddle_score, R_paddle_score;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (ball_move_counter == 0) begin
                ...
                if (!left_direction[1]) begin  // If movement is enabled (direction[0] = 0)
                    if (!left_direction[0]) begin  // Moving down (direction[1] = 0)
                        if (y_pos_L + PADDLE_HEIGHT + PADDLE_SPEED <= `BOTTOM_Y)
                            y_pos_L <= y_pos_L + PADDLE_SPEED;
                    end
                    else if (left_direction[0])begin  // Moving up (direction[1] = 1)
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (ball_move_counter == 0) begin
                // Check if paddle is within screen boundaries
                if (!right_direction[1]) begin  // If movement is enabled (direction[0] = 0)
                    if (!right_direction[0]) begin  // Moving down (direction[1] = 0)
                        if (y_pos_R + PADDLE_HEIGHT + PADDLE_SPEED <= `BOTTOM_Y)
                            y_pos_R <= y_pos_R + PADDLE_SPEED;
                    end
                    else if (right_direction[0]) begin  // Moving up (direction[1] = 1)
    ...
    assign right_paddle_score = R_paddle_score;  //Here I need to create scorekeeper
